      ...
      <div className="max-w-[1600px] mx-auto relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-8 h-[calc(100vh-100px)]">
        ...
        <div className="lg:col-span-4 flex flex-col gap-6 h-full overflow-y-auto pr-2 custom-scrollbar">
            ...
            <div className="flex flex-wrap gap-2 pt-1">
              {["Silver Hair", "Blue Eyes", "Cyberpunk", "Maid", "Gothic"].map((tag) => (
                <Badge 
                  key={tag} 
                  variant="outline" 
                  className="cursor-pointer bg-white/5 hover:bg-white/10 border-white/10 text-white/70 hover:text-white transition-all px-3 py-1 rounded-full text-xs hover:border-primary/30"
          ...
          <Button 
            ...
            className={cn(
              "w-full h-14 text-lg font-semibold shadow-xl transition-all duration-500 relative overflow-hidden group",
              isGenerating 
          ...
          >
            {isGenerating ? (
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-normal opacity-70 ml-1">(5 Credits)</span>
              </div>
            )}
          </Button>
          ...
          <div className="mt-auto pt-6 border-t border-white/10">
            <Label className="text-sm font-medium text-white/80 mb-3 block">Recent Creations</Label>
            <div className="flex gap-3 overflow-x-auto pb-2 custom-scrollbar snap-x min-h-[80px]">
              {history === undefined ? (
                 // Loading State
                 [1,2,3].map(i => (
                   <div key={i} className="min-w-[80px] h-20 rounded-lg bg-white/5 animate-pulse border border-white/5" />
                 ))
               ) : history.length === 0 ? (
                 // Empty State
                 <div className="flex items-center justify-center w-full text-white/30 text-sm h-20 border border-dashed border-white/10 rounded-lg">
                 ...
                 history.map((item) => (
                   <div 
                      ...
                      className={cn(
                          "min-w-[80px] h-20 rounded-lg overflow-hidden border border-white/10 flex-shrink-0 cursor-pointer hover:border-primary transition-all snap-start relative group",
                          generatedImage === item.image_url && "border-primary ring-1 ring-primary shadow-lg shadow-primary/20"
                      )}
                   >
                     <img src={item.image_url} alt="History" className="w-full h-full object-cover opacity-80 group-hover:opacity-100 transition-opacity" />
                     {item.status === 'failed' && (
        ...
        {/* RIGHT PANEL: PREVIEW */}
        <div className="lg:col-span-8 h-full bg-neutral-900/50 rounded-3xl border border-white/10 relative overflow-hidden flex items-center justify-center backdrop-blur-sm shadow-2xl">
            ...
            {generatedImage ? (
                // RESULT VIEW
                <div className="relative w-full h-full p-8 flex flex-col items-center justify-center animate-in zoom-in-95 duration-500">
